...
func (a *AnalyzeARP) GetDisplayInfo() string {
	...
	operation := getOrDefault(info["operation"], "不明")
	srcMac := getOrDefault(info["src_mac"], "不明")
	srcIP := getOrDefault(info["src_ip"], "不明")
	dstMac := getOrDefault(info["dst_mac"], "不明")
	dstIP := getOrDefault(info["dst_ip"], "不明")
...
func (a *AnalyzeARP) GetSummary() string {
	...
	srcIP := getOrDefault(info["src_ip"], "")
	dstIP := getOrDefault(info["dst_ip"], "")
