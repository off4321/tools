...
func (a *AnalyzeTCP) GetDisplayInfo() string {
	...
	flagsDesc, _ := info["flags_desc"].(string)
	if flagsDesc == "" {
		flagsDesc = "不明"
	}
	...
	ack, _ := info["ack"].(string)
	if ack == "" {
		ack = "N/A"
	}
	
	portInfo := fmt.Sprintf("%s -> %s", srcPort, dstPort)
	flagInfo := fmt.Sprintf("[%s]", flagsDesc)
	
	if strings.Contains(flagsDesc, "SYN") && !strings.Contains(flagsDesc, "ACK") {
		return fmt.Sprintf("TCP %s %s 接続開始", portInfo, flagInfo)
	} else if strings.Contains(flagsDesc, "SYN") && strings.Contains(flagsDesc, "ACK") {
		return fmt.Sprintf("TCP %s %s 接続確認", portInfo, flagInfo)
	} else if strings.Contains(flagsDesc, "FIN") {
		return fmt.Sprintf("TCP %s %s 接続終了", portInfo, flagInfo)
	} else if strings.Contains(flagsDesc, "RST") {
		return fmt.Sprintf("TCP %s %s 接続リセット", portInfo, flagInfo)
	} else if strings.Contains(flagsDesc, "ACK") {
...
func (a *AnalyzeTCP) GetSummary() string {
	...
	if serviceInfo != "" {
		return fmt.Sprintf("TCP %s -> %s (%s) %s", srcPort, dstPort, serviceInfo, flagsDesc)
	} else {
		return fmt.Sprintf("TCP %s -> %s %s", srcPort, dstPort, flagsDesc)
	}
}
